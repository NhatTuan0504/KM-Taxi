...
public class MqttService extends Service
{
	...
	private static final String 	MQTT_BROKER = "localhost"; // Broker URL or IP Address
    ...
    /**
     * send o sms the subscribed topic
     */
	public static void publishMessage(String sms, List<String> listTopic){
		if(mClient != null){
			MqttMessage message = new MqttMessage(sms.getBytes());
			message.setQos(2);
			message.setRetained(false);
			try {
				for (String deviceId : listTopic) {
					mClient.publish(deviceId, message);
				}
	...
	private synchronized void connect() {
		String url = "tcp://104.155.228.2:1883";//String.format(Locale.US, MQTT_URL_FORMAT, MQTT_BROKER, MQTT_PORT);
...
//					mClient.connect(mOpts);
					...
					mClient.connect();
					mClient.subscribe(MyApplication.getInstance().getDevideId(), 0);

					mClient.setCallback(new com.example.tuann.clientuser.interfaces.MqttCallback());
